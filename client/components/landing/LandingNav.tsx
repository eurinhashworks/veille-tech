    ...
    return (
        <nav className="container mx-auto px-6 py-4 flex justify-between items-center relative z-20">
            ...
            <div className="hidden md:flex items-center gap-10">
                <Link to={ROUTES.LANDING.path} className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary transition font-semibold">Accueil</Link>
                {/* Produit Dropdown */}
                <div className="relative group">
                    ...
                    <div className="absolute top-full left-0 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                        <div className="space-y-4">
                            <Link to={ROUTES.LOGIN.path} className="flex items-start gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl transition cursor-pointer">
                ...
                <Link to={ROUTES.ABOUT.path} className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary transition font-semibold">Entreprise</Link>
            ...
            <div className="flex items-center gap-6">
                <Link to={ROUTES.LOGIN.path} className="hidden sm:block text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-primary transition">
                    Se connecter
                </Link>
                <Link
